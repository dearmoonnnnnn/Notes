...
- 这个例子中，基类`Shape`有一个虚函数`draw()`,两个派生类分别重写了这个虚函数。
...
- 存在于每一个**非静态成员函数**中，可以在函数体里面使用，但是不能在参数中写出来，显示地作为参数传递给成员函数。(图中写出来只是为了方便理解)
- 静态成员函数不能使用this
...
String对象只提供对外接口，而字符串的所有实现交给StringRep这个类。
