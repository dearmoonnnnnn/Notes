...
- 转换完成后，从 `sensor_msgs::PointCloud2` 重新转换为 `pcl::PointCloud<pcl::PointXYZIDV>`，并读取多普勒速度和信号强度，值均为 0 

##### 错误原因

- `sensor_msgs::PointCloud2` 中的字段 `intensity` 和 `doppler_velocity` 的偏移量为 8 （字节）
  - 8 字节使用的数据类型为 `float64`
  - 定义点时的数据类型为 `float`，即 `float32`
